...
public class PasswordServiceImpl extends ServiceImpl<PasswordMapper, Password> implements PasswordService {
    ...
    @Override
    public Result<IPage<Password>> getPagePasswords(Integer current, Integer size) {
        IPage<Password> passwordIPage = new Page<>(current, size);
        IPage<Password> pageData = page(passwordIPage);
        if (ObjectUtil.isNotEmpty(pageData)) {
            return Result.success(pageData, "获取分页数据成功!", null);
        }
        return Result.failure(null, "获取分页数据失败！", null);
    }
